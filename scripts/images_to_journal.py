...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if len(sys.argv) > 2 and sys.argv[2] == "recurse":
            recurse = True
    ...
    j = node.create_journal(full_path=full_path)
    ...
    #j.to_file('temp.txt', sort="chronological")

    if recurse:
        for dpath in node.directories:
            d = dpath.load()
            j = d.create_journal(full_path=full_path)
